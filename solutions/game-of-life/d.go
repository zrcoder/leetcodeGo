...
func calLives(board [][]int, r, c int) int {
	lives := 0
	for _, d := range dirs {
		if isLive(board, r+d[0], c+d[1]) {
			lives++
		}
	}
	return lives
...
func isLive(board [][]int, r, c int) bool {
	m, n := len(board), len(board[0])
	return r >= 0 && r < m && c >= 0 && c < n && (board[r][c] == 1 || board[r][c] == 2)
}
